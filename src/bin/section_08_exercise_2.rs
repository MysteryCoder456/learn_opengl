// Getting Started - Section 8: Transformations - Exercise 2
...
fn main() {
    ...
    let shader_program = unsafe {
        let shader = Shader::new(
            "shaders/section_8/vertex.glsl",
            "shaders/section_8/fragment.glsl",
        );
    ...
    while !window.should_close() {
        ...
        let mut first_transform = nalgebra_glm::Mat4::identity();
        first_transform =
            nalgebra_glm::translate(&first_transform, &nalgebra_glm::vec3(0.5, -0.5, 0.0));
        first_transform =
            nalgebra_glm::rotate(&first_transform, time, &nalgebra_glm::vec3(0.0, 0.0, 1.0));

        let mut second_transform = nalgebra_glm::Mat4::identity();
        second_transform =
            nalgebra_glm::translate(&second_transform, &nalgebra_glm::vec3(-0.5, 0.5, 0.0));
        let sin_time = time.sin();
        second_transform = nalgebra_glm::scale(
            &second_transform,
            &nalgebra_glm::vec3(sin_time, sin_time, 1.0),
        );
        ...
        unsafe {
            ...
            // Configure transform location and draw the vertex array for first container
            gl::UniformMatrix4fv(
                shader_program.get_uniform_location("transform"),
                1,
                gl::FALSE,
                nalgebra_glm::value_ptr(&first_transform).as_ptr() as *const GLfloat,
            );
            gl::DrawElements(gl::TRIANGLES, 6, gl::UNSIGNED_INT, 0 as *const c_void);

            // Configure transform location and draw the vertex array for second container
            gl::UniformMatrix4fv(
                shader_program.get_uniform_location("transform"),
                1,
                gl::FALSE,
                nalgebra_glm::value_ptr(&second_transform).as_ptr() as *const GLfloat,
            );
            gl::DrawElements(gl::TRIANGLES, 6, gl::UNSIGNED_INT, 0 as *const c_void);
        }
